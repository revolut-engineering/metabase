...
(defn connection-pool
  ...
  {:datasource (doto (ComboPooledDataSource.)
                 ...
                 (.setMaxIdleTimeExcessConnections (if (str/includes? subprotocol "exa") (* 15 60) (* 30 60)))
                 (.setMaxIdleTime                  (if (str/includes? subprotocol "exa") (* 30 60) (* 60 60)))
                 (.setMaxConnectionAge             (if (str/includes? subprotocol "exa") (* 60 60) 0))
                 (.setInitialPoolSize              3)
                 (.setMinPoolSize                  minimum-pool-size)
                 (.setMaxPoolSize                  (if (str/includes? subprotocol "exa") 12 15))
                 (.setAcquireIncrement             3)
                 (.setNumHelperThreads             (if (str/includes? subprotocol "exa") 5 1))
